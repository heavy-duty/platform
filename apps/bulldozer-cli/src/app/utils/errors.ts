...
export interface BulldozerError {
	status?: number;
	response?: {
		data: {
			message: string;
	...
	logs?: string[];
	bulldozerError?: boolean;
	message?: string;
}

// TODO: Refact
export const processError = (e: BulldozerError) => {
	switch (true) {
		...
		case e.logs !== undefined:
			...
			break;
		case e.bulldozerError !== undefined:
			log('❌❌ A Bulldozer Cli error occurred:');
			log('> ' + e.message);
			log(
