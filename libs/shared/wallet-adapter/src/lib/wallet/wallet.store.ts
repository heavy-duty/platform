...
export class WalletStore extends ComponentStore<WalletState> {
    ...
    (publicKey, adapter, connected) => {
      const adapterSignTransaction =
        adapter && 'signTransaction' in adapter
          ? signTransaction(adapter, connected, this._error)
          : undefined;
  ...
  readonly autoConnect = this.effect(() => {
    return combineLatest([
      ...
      filter(
        ([autoConnect, , ready, connecting, connected]) =>
          autoConnect && ready && !connecting && !connected
      ),
      concatMap(([, adapter]) => {
  ...
  readonly selectWallet = this.effect(
    (newName$: Observable<WalletName | null>) => {
      return newName$.pipe(
        concatMap((action) =>
          of(action).pipe(withLatestFrom(this.name$, this.adapter$))
        ),
  ...
  readonly onReady = this.effect(() => {
    return this.adapter$.pipe(
      isNotNull,
      switchMap((adapter) =>
        fromAdapterEvent(adapter, 'ready').pipe(
  ...
  signMessage(message: Uint8Array): Observable<Uint8Array> | undefined {
    ...
    return adapter && 'signMessage' in adapter
      ? signMessage(adapter, connected, this._error)(message)
      : undefined;
  }
}
